...
Store data is built around the three basic objects in the backend:
- categories
- posts: each post belongs to a category
- comments: each comment belongs to a post
...
Categories are not duplicated in the store. The current app allows navigating through posts and comments. Categories can be read through a dedicated API endpoint. Categories visible to the frontend are currently obtained by reducing over posts. This limits app categories to those known through whichever posts have been fetched. (So far, the app fetches every single post.) The plan to build out categories will expand the store to include an additional reducer:
```
// TODO: include categories in store for increased functionality around them
{
	posts: {},
	comments: {},
	categories: {}
...
The existing API return for a category has the following structure:
```
{
	name: '',
	path: ''
...
The API server is available for local development and testing. There are three types of objects stored on the server:
- Categories
- Posts
- Comments
...
## Components
The application was initially developed with four views: root, category, post detail, create/edit and post/comment.

### Container components
- to fill out as built
